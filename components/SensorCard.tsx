...
export function SensorCard({ nodeId, moisture, temperature, humidity }: SensorCardProps) {
  ...
  return (
    <Link href={`/sensors/${nodeId}`}>
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 border border-green-400 shadow-green-200 cursor-pointer hover:shadow-lg transition-shadow">
        <div className="flex justify-between items-start mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Plant {nodeId.replace('Node_', '')}
            </p>
          </div>

          <span
            className={`px-3 py-1 rounded-full text-sm font-medium ${condition.color}`}
          >
            {condition.label}
          </span>
        </div>
